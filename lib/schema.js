...
export default class Schema {
   ...
   */
  static FILTER_OPERATORS = new Set(['$eq', '$ne', '$gt', '$gte', '$lt', '$lte', '$between', '$like']);

  /**
   * Build WHERE clauses and parameters from a filter object that supports
   * MongoDB-style operators.
   *
   * Plain values are treated as exact match ($eq). Object values with
   * operator keys ($gte, $lte, $gt, $lt, $ne, $between, $like) generate
   * the corresponding SQL conditions.
   *
   ...
   */
  static buildFilterClauses (model, filter, schema) {
    let whereClauses = '';
    let params = {};
    ...
    for (const field in filter) {
      ...
      // If value is an object with operator keys, handle as operator filter
      if (value !== null && value !== undefined && typeof value === 'object' && !Array.isArray(value)) {
        const operators = Object.keys(value).filter(k => Schema.FILTER_OPERATORS.has(k));
        if (operators.length > 0) {
          for (const op of operators) {
            const opValue = value[op];
            let clause = '';
            // Use operator-specific suffix to avoid parameter key collisions
            // when multiple operators are used on the same field (e.g., $gte + $lte)
            const opSuffix = op.replace('$', '');
            if (op === '$eq') {
              const key = Schema.paramKey(`${field}_${opSuffix}`);
              clause = `${field} = ${key}`;
              params[key] = Schema.serializeValue(field, opValue, schema);
            } else if (op === '$ne') {
              const key = Schema.paramKey(`${field}_${opSuffix}`);
              clause = `${field} != ${key}`;
              params[key] = Schema.serializeValue(field, opValue, schema);
              ...
              if (!Array.isArray(opValue) || opValue.length !== 2) {
                throw new Error(`Operator '$between' on field '${field}' requires an array of exactly two values [low, high].`);
              }
              const keyLow = Schema.paramKey(`${field}_${opSuffix}_low`);
              const keyHigh = Schema.paramKey(`${field}_${opSuffix}_high`);
              clause = `${field} BETWEEN ${keyLow} AND ${keyHigh}`;
              params[keyLow] = Schema.serializeValue(field, opValue[0], schema);
              params[keyHigh] = Schema.serializeValue(field, opValue[1], schema);
            } else if (op === '$like') {
              const key = Schema.paramKey(`${field}_${opSuffix}`);
              clause = `${field} LIKE ${key}`;
              params[key] = Schema.serializeValue(field, opValue, schema);
            }
            if (clause) {
              whereClauses += `${whereClauses.length ? ' AND ' : ''}${clause}`;
            }
          }
        } else {
          // Object without recognized operators — treat as exact match for backward compatibility
          const key = Schema.paramKey(field);
          whereClauses += `${whereClauses.length ? ' AND ' : ''}${field} = ${key}`;
          params[key] = Schema.serializeValue(field, value, schema);
        }
      } else {
  ...
  // search records with filter and pagination via POST body
  static searchRecords (model, filter, database, options = {}) {
    Schema.validateModelName(model);
    const schema = Schema.getSchema(model, database);
    const { page, limit } = options;

    const { whereClauses, params } = Schema.buildFilterClauses(model, filter, schema);

    let baseQuery = whereClauses
      ? `SELECT * FROM ${model} WHERE ${whereClauses}`
      : `SELECT * FROM ${model}`;

    // Get total count (without pagination)
    let countQuery = whereClauses
      ? `SELECT COUNT(*) AS count FROM ${model} WHERE ${whereClauses}`
      : `SELECT COUNT(*) AS count FROM ${model}`;
    let countStatement = database.prepare(countQuery);
    let countResult = countStatement.get(params);
    const totalCount = countResult ? countResult.count : 0;

    // Apply pagination if valid page/limit provided
    let query = baseQuery;
    let records = [];
    if (page !== null && !isNaN(page) && limit !== null && !isNaN(limit) && page > 0 && limit > 0) {
      const offset = (page - 1) * limit;
      query += ` LIMIT ${limit} OFFSET ${offset}`;
    }

    let statement = database.prepare(query);
    records = statement.all(params);

    // Coerce values back to their schema-defined types
    if (schema && records && records.length > 0) {
      records = records.map(row => Schema.coerceRow(row, schema));
    }
  ...
  // get total count of records matching field criteria
  static countRecords (model, fields, database) {
    Schema.validateModelName(model);
    const schema = Schema.getSchema(model, database);
    let whereClauses = '';
    let params = {};
