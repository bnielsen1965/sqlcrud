...
export default class Schema {
  ...
  static updateRecord (model, fields, record, database) {
    ...
    const before = Schema.coerceRow(existing[0], schema);
    ...
    // Fetch the updated row so the caller sees the actual persisted data
    let fetchStatement = database.prepare(`SELECT * FROM ${model} WHERE ${whereClauses}`);
    let updated = fetchStatement.all(whereParams);
  ...
  /**
   * Serialize a value for storage in SQLite based on the schema type.
   * JSON-type objects and arrays are stringified; all other types pass through.
